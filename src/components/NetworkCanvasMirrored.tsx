...
const NetworkCanvasMirrored = () => {
  ...
  const generateNodes = useCallback((w: number, h: number): Node[] => {
    ...
    // 3 clean dots at bottom-left, horizontally spaced (mirrored)
    const dotsY = h * 0.55;
    const dotsStartX = w * 0.45;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Curves from cluster to dots (mirrored: favor left-side exit nodes)
      const sortedByExit = [...clusterNodes].sort((a, b) => {
        const scoreA = (w - a.x) * 0.7 + a.y * 0.3;
        const scoreB = (w - b.x) * 0.7 + b.y * 0.3;
...
export default NetworkCanvasMirrored;
